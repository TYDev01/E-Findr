...
- `docker-compose.yml`: Local orchestration for frontend, API, AI service, PostgreSQL + pgvector, and Redis.
...
- Event creation, listing, detail, update, and deletion routes
...
- The upload route stores originals and thumbnails in R2, records metadata in PostgreSQL, pushes Redis jobs, and immediately triggers internal processing for local development feedback.
...
- The backend event routes themselves still do not enforce organizer ownership yet, so API-level authorization is the next hardening step after this phase.
